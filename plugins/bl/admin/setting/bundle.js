...
function updateIPCData(row, id, data) {
	...
	if (!data.heartbeat || time < 4) {
		row.find('.client-status').removeClass('error warning').text('IPC Connected ' + time);
	} else if (time < 45) {
		row.find('.client-status').removeClass('error').addClass('warning').text('IPC Warning ' + time);
	} else {
		...
		row.find('.client-status').removeClass('warning').addClass('error').text('IPC Dead ' + time);
	}
...
function addClientRow(botid) {
    ...
    actions.append($('<input>').attr('class', 'btn btn-info').attr('type', 'button').attr('value', 'Restart').on('click', restartButtonCallback));
    actions.append($('<input>').attr('class', 'btn btn-danger').attr('type', 'button').attr('value', 'Terminate').on('click', terminateButtonCallback));
    row.append(actions);
